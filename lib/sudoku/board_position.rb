module Sudoku
  class BoardPosition
	...
	# Public: iterates over each position in a row.
	#
	# y - y position of this row
	# width - width of this row
	# block - code block to iterate over each row position.
	#
	# Returns nothing.
	def self.each_row_position(y, width, &block)
	  if ! block_given?
		raise RuntimeError, "no block passed to #each_row"
	  end
	  
	  [0..width-1].each do |x|
		position = self.at( x, y )
		block.call(position)
	  end
	end

	# Public: iterates over each position in a column.
	#
	# x - x position of this column
	# height - height of this column
	# block - code block to iterate over each row position.
	#
	# Returns nothing.
	def self.each_column_position(x, height, &block)
	  if ! block_given?
		raise RuntimeError, "no block passed to #each_row"
	  end
	...
	def self.at(x, y)
	  ...
	  @@existing_positions[x][y] ||= self.at(x,y)
